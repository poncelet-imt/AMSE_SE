...
int main(int argc, char *argv[])
{
    ...
    double errorDistance; /* different entre la ditance parcourue et  */

    double distance; /* Distance a parcourir */
    ...
    if (sscanf(argv[1], "%lf", &distance)  == 0) 
    {
        fprintf(stderr,"ERREUR : ---> parametre 1: Distance a parcourir doit etre un double\n");
        return 1;
    }
    ...
    errorDistance = sqrt((x_0 - *x)*(x_0 - *x) + (y_0 - *y)*(y_0 - *y)) - distance;
    /* affichage + calcul */

    do
    {
        errorDistance = distance - sqrt((x_0 - *x)*(x_0 - *x) + (y_0 - *y)*(y_0 - *y));
        if (errorDistance > 0.0)
        {
            *tvR = w;
            *tvL = w;
        } else if (errorDistance < 0.0)
        {
            *tvR = -w;
            *tvL = -w;
        } else {
    ...
    while( g_run && (fabs(errorDistance) > EPSILON) );
